...
import { Package, CheckCircle, RefreshCw, Store as StoreIcon, ChevronDown, ChevronUp, Calendar, User, FileText } from 'lucide-react'
  ...
  const loadTransfers = async () => {
    ...
    try {
      const [pending, received] = await Promise.all([
        StoreStockTransferService.getPendingTransfers(currentStoreId),
        StoreStockTransferService.getReceivedTransfers(currentStoreId)
      ])
  ...
  return (
    <RoleProtectedRoute module="products" requiredAction="view">
      <div className="p-4 md:p-6 pb-20 lg:pb-6 space-y-4 md:space-y-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
        ...
        {(() => {
          const transfersToShow = filter === 'all' 
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">
                      {filter === 'pending'
                        ...
                        : filter === 'received'
                        ? 'Aún no has recibido ninguna transferencia'
